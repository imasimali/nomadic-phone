...
// SMS API
export const smsAPI = {
  sendSMS: (to: string, body: string, mediaUrls?: string[]) =>
    api.post('/sms/send', { to, body, mediaUrls }),
  
  getMessages: (params?: { page?: number; limit?: number; direction?: string; status?: string }) =>
    api.get('/sms/messages', { params }),
  
  getMessage: (messageSid: string) => api.get(`/sms/messages/${messageSid}`),
  
  getConversation: (phoneNumber: string, params?: { page?: number; limit?: number }) =>
    api.get(`/sms/conversations/${encodeURIComponent(phoneNumber)}`, { params }),
