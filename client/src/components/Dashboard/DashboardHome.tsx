  ...
  const loadDashboardData = async () => {
    try {
      const [callsResponse, smsResponse] = await Promise.all([
        voiceAPI.getCalls({ limit: 5 }),
        smsAPI.getMessages({ limit: 5 }),
      ]);
            ...
            <CardContent>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                ...
                <Chip
                  ...
                  color={voiceStatus.color as any}
                  size="small"
                  sx={{ ml: 1 }}
                />
              </Box>
                  ...
                  {stats.recentSMS.map((sms, index) => (
                    <React.Fragment key={sms.id}>
                      <ListItem>
                        <ListItemIcon>
                          <Message 
                            color={sms.direction === 'outbound' ? 'primary' : 'secondary'} 
                          />
                        </ListItemIcon>
                        <ListItemText
                          primary={formatPhoneNumber(
                            sms.direction === 'outbound' ? sms.to_number : sms.from_number
                          )}
                          secondary={
                            <React.Fragment>
