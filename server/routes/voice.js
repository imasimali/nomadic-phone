...
// Hangup a call
router.post('/calls/:callSid/hangup', asyncHandler(async (req, res) => {
  const { callSid } = req.params;
...
router.put('/settings', [
  body('incoming_call_action')
    .optional()
    .isIn(['recording', 'client', 'redirect'])
    .withMessage('Invalid incoming call action'),
  ...
  body('voice_message')
    ...
    .withMessage('Voice message must be 1-500 characters'),
], asyncHandler(async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).json({
...
router.get('/recordings/:recordingSid', asyncHandler(async (req, res) => {
